          ...
          BlocBuilder<WorkoutBloc,WorkoutState>(
            builder:(context,state){
                if(state is LoadingExerciseState){
                  return const Center(child: CircularProgressIndicator(color:  Color.fromARGB(255,3,118,77),));
...
class _List_view extends StatelessWidget{
  ...
  Widget list_item(index){
    return Container(
     // width: 250,
      child: ListTile(
        title: Text(bloc.exercise_list[index].exerciseName!,style: TextStyle(
          color: Colors.white,fontSize: 20,fontWeight: FontWeight.bold),),
        subtitle: Text(bloc.exercise_list[index].description!,style: TextStyle(
